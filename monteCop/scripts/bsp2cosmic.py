...
'''
    ...
    Examples:
        >> bsp2cosmic.py gen_LLO_to_NRHO_imp_ext7d_BSP.bsp -ov -o 3 -tl 1 -dt 10 -dv 20
...
# a) Using getattr
# Call function dynamically by name:
# func = getattr(obj, "dostuff")

# b) Using nameToCoord:
# c = Coordinate.nameToCoord( 'conic.semiMajorAxis' )
## Name of coordinate NOt case sensitive

# -----------------------------------------------------------------------
#  Note on naturalCenter:
# -----------------------------------------------------------------------
# TrajSetBoa.read(boa).naturalCenter('mySC')
# TrajSetBoa.read(mgr.boa).naturalCenter('mySC', t0_Epoch)

# Read Boa State without query:
# TrajSetBoa.read(boa).state(ti,'encnf5','Enceladus','EME2000')
# state( Epoch requestTime, str body, str center, str coordFrame,ParamList q, [int dim] )

# -----------------------------------------------------------------------
#  Find Events:
# -----------------------------------------------------------------------

# ev = ApsisEvent( TrajQuery( boa, "Moon", "Earth" ), ApsisEvent.APOAPSIS )
# r = ev.search( TimeInterval( "1-OCT-2010 ET", "1-NOV-2010 ET" ), 5*day )
# moonApo = r[0].time()
